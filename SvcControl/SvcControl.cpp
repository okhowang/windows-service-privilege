...
class Service {
    ...
    void QuerySecurity() {
        ...
        if (!QueryServiceObjectSecurity(handle,
            ...
            psd.get(),           // using NULL does not work on all versions
            sizeof *psd.get(),
            &dwBytesNeeded))
        {
            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER)
            {
                ...
                if (!QueryServiceObjectSecurity(handle,
                    DACL_SECURITY_INFORMATION, psd.get(), dwSize, &dwBytesNeeded))
                {
        ...
        if (bDaclPresent) {
            ...
            for (auto i = 0; i < count; i++) {
                ...
                case TRUSTEE_IS_SID:
                {
                    ...
                    SID_NAME_USE use;
                    LookupAccountSid(NULL, sid, name, &szName, domain, &szDomain, &use);
                    wprintf(L" %s:%s:%d ", domain, name, use);
...
{
    try {
        ServiceControlManager().QueryConfig(L"SharedAccess");
    }
